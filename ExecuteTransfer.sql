...
        -- 0. Check Idempotency FIRST to prevent duplicate processing crashes
        IF EXISTS (SELECT 1 FROM IdempotencyRecords WHERE [Key] = @IdempotencyKey)
        BEGIN
            RAISERROR('Duplicate request detected: This transaction has already been processed.', 16, 1);
